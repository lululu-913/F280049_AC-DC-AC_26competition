...
$busReference = Read-Float 'U_BUS_REF\s*=\s*([0-9.]+)f' '50 V bus reference'
...
$inputOvercurrentLimit = Read-Float 'INPUT_OVERCURRENT_LIMIT\s+([0-9.]+)f' '7 A input overcurrent limit'
...
if ([Math]::Abs($busReference - 50.0) -gt 1.0e-4) {
    throw "Bus reference is $busReference V instead of 50 V."
}
...
if ([Math]::Abs($overvoltage - ($busReference + 10.0)) -gt 1.0e-4) {
    throw "Bus protection must be exactly 10 V above the nominal bus reference."
}
if ([Math]::Abs($inputCurrentPeakMaximum - 6.0) -gt 1.0e-4 -or
    [Math]::Abs($inputOvercurrentLimit - 7.0) -gt 1.0e-4) {
    throw 'The input-current reference limit must be 6 A and overcurrent protection must remain 7 A.'
}
...
if ($source -notmatch 'case\s+KEY1_PRESS:[\s\S]*?U_BUS_REF\s*\+=\s*BUS_REF_STEP;[\s\S]*?if\s*\(\s*U_BUS_REF\s*>\s*55\.0f\s*\)\s*U_BUS_REF\s*=\s*55\.0f;[\s\S]*?break;') {
    throw 'KEY1 must clamp the bus reference before the ISR can observe a value above 55 V.'
}
...
if ($busReference -le $rectifierMinimumBus) {
    throw 'The 50 V bus lacks boost-rectifier voltage headroom.'
}
if ($requiredModulationDivisor -le $modulationMinimum) {
    throw 'The nominal 50 V bus leaves no SVPWM modulation-divisor margin.'
}
...
Write-Output (
    'PASS: 24 Vrms input, 30 Vrms line output, 50 V bus, 60 V protection; nominal M={0:F3}.' -f
