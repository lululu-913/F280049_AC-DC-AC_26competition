...
#define BUS_CONTROL_MIN_VOLTAGE 2.0f                                        // 母线电压归一化分母下限，避免启动时除数过小（功率减半）
#define RECTIFIER_VOLTAGE_KP 0.075f                                        // 整流母线电压外环比例增益
...
#define INVERTER_MODULATION_DIVISOR_MAX 8.0f                               // 逆变调制分母上限
#define OUTPUT_PHASE_REF_MAX_RMS 20.0f                                     // 输出相电压参考上限，覆盖32Vrms线电压目标
...
float M = INVERTER_MODULATION_DIVISOR_INITIAL;                             // 逆变电压环调制分母从集中宏读取上电初值
...
float U_bus_ff = 55.0f;                                                     // 逆变瞬时母线前馈滤波状态从额定母线值开始
...
float v_dc_bus = 5.0f;                                                      // 单极性调制归一化使用的实时母线电压（功率减半）
...
float kp = 100.0f;                                                          // 20kHz采样下的PLL每拍积分增量系数
float ki = 10.0f;                                                           // PLL误差差分项系数与采样周期无关，保持不变
...
__interrupt void adcA1ISR(void)                                             // 20kHz ADC中断与全部实时控制算法
{
    ...
    if(output_freq_actual_hz < output_freq_target_hz)                       // 升频时按固定斜率逐拍逼近目标值
    {
        output_freq_actual_hz += OUTPUT_FREQ_RAMP_STEP_HZ;                  // 20kHz下30Hz频差约0.993秒完成
        if(output_freq_actual_hz > output_freq_target_hz)                   // 最后一拍避免浮点步进越过目标值
            output_freq_actual_hz = output_freq_target_hz;                  // 精确停在30Hz或60Hz目标值
    }
    else if(output_freq_actual_hz > output_freq_target_hz)                  // 降频时保持相同的30Hz/s斜率
    {
        output_freq_actual_hz -= OUTPUT_FREQ_RAMP_STEP_HZ;                  // 每拍降低0.00151Hz并保持相位连续
        if(output_freq_actual_hz < output_freq_target_hz)                   // 最后一拍避免低于目标值
            output_freq_actual_hz = output_freq_target_hz;                  // 精确停在30Hz或60Hz目标值
    }

    if((inverter_pwm_start_stage != 0) && (system_fault == 0))              // 逆变处于预装、待释放或正常运行状态
    {
        theta_a += 2.0f * pi * output_freq_actual_hz * Ts;                  // 每个20kHz控制周期按斜坡后的实际频率连续推进输出相位
        ...
        EPwm1Regs.CMPA.bit.CMPA = (Uint16)(INVERTER_EPWM_TBPRD * Da);         // 将SVPWM A相占空比转换为中心对齐比较值（20kHz）
        EPwm2Regs.CMPA.bit.CMPA = (Uint16)(INVERTER_EPWM_TBPRD * Db);         // 将SVPWM B相占空比转换为中心对齐比较值（20kHz）
        EPwm3Regs.CMPA.bit.CMPA = (Uint16)(INVERTER_EPWM_TBPRD * Dc);         // 将SVPWM C相占空比转换为中心对齐比较值（20kHz）
        ...
        if(N_c2 >= N)                                                       // 每200个20kHz采样点更新一次慢速控制状态
        {
            ...
            if(M >= INVERTER_MODULATION_DIVISOR_MAX) M = INVERTER_MODULATION_DIVISOR_MAX;                                               // 保留原调制分母上限
            if(M <= INVERTER_MODULATION_DIVISOR_MIN) M = INVERTER_MODULATION_DIVISOR_MIN;                                         // SVPWM下限保留约1.9%最小占空比裕量
...
void PID1_Init(void)                                                        // 初始化母线电压外环参数
{
    ...
    pid1.Kp = RECTIFIER_VOLTAGE_KP;                                           // 从集中宏读取母线电压外环比例增益
    pid1.Ki = RECTIFIER_VOLTAGE_KI;                                           // 从集中宏读取母线电压外环积分增益
...
float PID2_Cal(float u)                                                     // 计算输入电流PI控制输出
{
    ...
    if(pid2.Integral > RECTIFIER_CURRENT_INTEGRAL_LIMIT)                    // 限制正向积分状态
        pid2.Integral = RECTIFIER_CURRENT_INTEGRAL_LIMIT;                   // 最大积分补偿为0.6V
    else if(pid2.Integral < -RECTIFIER_CURRENT_INTEGRAL_LIMIT)              // 限制反向积分状态
        pid2.Integral = -RECTIFIER_CURRENT_INTEGRAL_LIMIT;                  // 最小积分补偿为-0.6V
...
void PIDa_Init(void)                                                        // 初始化逆变输出电压控制参数
{
    ...
    pida.Kp = INVERTER_VOLTAGE_KP;                                            // 从集中宏读取逆变电压环比例增益
    pida.Ki = INVERTER_VOLTAGE_KI;                                            // 从集中宏读取逆变电压环积分增益
...
float PIDa_Cal(float u)                                                     // 计算逆变调制分母增量
{
    ...
    if(fabsf(pida.Err) > INVERTER_VOLTAGE_ERROR_DEADBAND) return pida.result;                         // 输出误差超出死区时返回调节增量
...
static void PWM_TripInverter(void)                                          // 只关断逆变侧ePWM1～3并保持整流侧当前状态
{
    ...
    inverter_pwm_release_wait = 0U;                                         // 每次Trip后重新等待完整PWM周期再释放
    EDIS;                                                                   // 重新禁止访问受保护寄存器
}
...
static void PWM_ReleaseInverter(void)                                       // 按逆变独立启动阶段释放ePWM1～3
{
    ...
    {
        inverter_pwm_release_wait = 0U;                                     // 从CMPA预装完成时开始计算等待拍数
        inverter_pwm_start_stage = 2;                                       // 等待一个完整20kHz PWM周期后再释放Trip
    }
    else if(inverter_pwm_start_stage == 2)                                  // CMPA预装后等待一个完整逆变PWM周期
    {
        if(inverter_pwm_release_wait < INVERTER_PWM_RELEASE_WAIT_ISR)        // 使用饱和计数避免等待变量回绕
            inverter_pwm_release_wait++;                                    // 20kHz ISR累计两拍即100us
        if(inverter_pwm_release_wait >= INVERTER_PWM_RELEASE_WAIT_ISR)       // 确认已跨过一次20kHz载波归零装载
        {
